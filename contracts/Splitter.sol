...
contract Splitter is Owner, Pause, Kill{
    ...
    function split(address bob, address carol) public payable onlyOwner whenNotPaused returns(bool success){
     ...
     require(bob != address(0x0) && carol != address(0x0), "You need to pass valid addresses");
        ...
        if(remainder != 0){
          balances[msg.sender] = balances[msg.sender].add(remainder).sub(msg.value);
        }
      ...
      function withdraw() public onlyOwner whenNotPaused returns(bool success) {
